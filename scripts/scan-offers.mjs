...
import { CHAIN_REGISTRY, chainFor } from '../connectors/registry.mjs';
import { connectorCatalog } from '../connectors/engine.mjs';
...
const CATALOG_OUTPUT=new URL('../data/connettori.json',import.meta.url);
...
const notExpired=o=>{const raw=o.validTo||o.validUntil||'';if(!raw)return true;const t=Date.parse(raw);return !Number.isFinite(t)||t>=today-86400000};
const offers=uniqueOffers([...localResults,...fallback]).filter(notExpired).sort((a,b)=>String(a.store).localeCompare(String(b.store),'it')||String(a.product).localeCompare(String(b.product),'it'));
...
const catalog={generatedAt:new Date().toISOString(),version:'5.0.0',connectors:connectorCatalog(CHAIN_REGISTRY,['penny','eurospin','lidl']),selectedChains:chains};
...
const snapshot={generatedAt:new Date().toISOString(),familyCode,total:offers.length,byChain:Object.fromEntries([...new Set(offers.map(o=>o.chain||o.store))].map(c=>[c,offers.filter(o=>(o.chain||o.store)===c).length]))};
history.snapshots=[snapshot,...(Array.isArray(history.snapshots)?history.snapshots:[])].slice(0,60);
await fs.writeFile(HISTORY_OUTPUT,JSON.stringify(history,null,2)+'\n','utf8');
